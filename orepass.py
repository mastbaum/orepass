...
couch = core.resource.Resource('http://adminuser:adminpass@localhost:5984')
...
handler = {
    # authentication
    r'^_cookie\/?$': handlers.auth.cookie_me,
    ...
    # document
    r'^(?P<dbname>\w+)\/(?!_)(?P<docid>\w+)\/?$': handlers.document.root,
...
def main(env, start_response):
    ...
    renderer = core.render.Renderer(start_response)
